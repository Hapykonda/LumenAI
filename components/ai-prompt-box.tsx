...
// Inject styles only in the browser. The 21st.dev component is client-side,
// but module scope still runs during Next's server build.
if (typeof document !== "undefined" && !document.getElementById("lumen-ai-prompt-box-styles")) {
  const styleSheet = document.createElement("style");
  styleSheet.id = "lumen-ai-prompt-box-styles";
...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  const visualizerStyleItems = React.useMemo(
    () =>
      Array.from({ length: visualizerBars }, (_, i) => ({
        height: `${18 + ((i * 37) % 78)}%`,
        animationDelay: `${i * 0.05}s`,
        animationDuration: `${0.5 + ((i * 17) % 45) / 100}s`,
      })),
    [visualizerBars]
  );
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-10 flex items-center justify-center gap-0.5 px-4">
        {visualizerStyleItems.map((style, i) => (
          <div
            key={i}
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  const setInputValue = React.useCallback(
    (nextValue: string) => {
      if (value === undefined) setInput(nextValue);
  ...
  return (
    <>
      ...
      >
              ...
              <div key={index} className="relative group">
                {file.type.startsWith("image/") && filePreviews[file.name] && (
                  <>
                    ...
                    aria-label={`Abrir vista previa de ${file.name}`}
                  >
                    <Image
        ...
        >
          <PromptInputTextarea
            aria-label="Mensaje para LumenAI"
            placeholder={
              showSearch
                ? "Search the web..."
                : showThink
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          ...
          >
            ...
            <div className="flex items-center">
              <button
                type="button"
                aria-label="Alternar búsqueda web"
              ...
              <button
                type="button"
                aria-label="Alternar razonamiento profundo"
